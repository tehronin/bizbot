...
function KeyValueContentView({ content }: { content: SidecarKeyValueContent }) {
  return (
...
function SidecarContentView({ content }: { content: SidecarContent }) {
  switch (content.type) {
    ...
    case "key_value":
      return <KeyValueContentView content={content} />;
    case "progress":
...
function SidecarPanelContentView({ panel, pending, error }: { panel: SidecarPanel; pending: boolean; error: string | null }) {
  if (panel.content.type === "selection") {
    return <SidecarSelectionView panelId={panel.panelId} content={panel.content} pending={pending} error={error} />;
  }
...
export default function SidecarHost() {
  ...
  async function requestSidecarState(operation: "back" | "close" | "activate", panelId?: string): Promise<void> {
    ...
    try {
      ...
      const payload = await response.json() as { error?: string; action: "close" | "update"; panel: SidecarPanel | null; stack?: { panels: SidecarPanel[]; activePanelId: string | null } };
      if (!response.ok) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 flex max-w-full" aria-label="Sidecar split view">
      {isExpanded ? (
        <button
          type="button"
      ...
      {isExpanded ? (
        <aside className="h-full border-l border-border bg-surface shadow-[0_0_24px_rgba(0,0,0,0.12)]" style={{ width: `min(${width}px, calc(100vw - 40px))` }}>
          <div className="sticky top-0 z-10 border-b border-border bg-surface px-5 py-4">
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-2 min-w-0 flex-1">
                <div className="text-xs font-mono uppercase tracking-widest text-muted">sidecar</div>
                <div className="text-sm font-medium break-words text-primary">{activePanel?.title ?? "Split view ready"}</div>
                <div className="text-[11px] uppercase tracking-[0.16em] text-dim">{activePanel?.content.type ?? "empty"}</div>
                {panels.length > 1 ? (
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => void goBack()}
                  disabled={isClosing || panels.length <= 1}
                  className="px-3 py-2 border border-border text-primary text-xs uppercase tracking-[0.18em] disabled:opacity-50"
                >
                  back
                </button>
          ...
          <div className="h-[calc(100vh-92px)] overflow-auto p-5">
            {activePanel ? <SidecarPanelContentView panel={activePanel} pending={interactionPending} error={interactionError} /> : <SidecarEmptyState />}
          </div>
        </aside>
      ) : null}

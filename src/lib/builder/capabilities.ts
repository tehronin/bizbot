...
export const BUILDER_CAPABILITY_CATALOG: BuilderCapabilityDefinition[] = [
  ...
  },
  {
    ...
    tier: "core",
    status: "available",
    summary: "First-class local git inspection and mutation workflows inside Builder-managed repos, with compatibility aliases during the tool-surface transition.",
    tools: [
      "builder_repo_status",
      "builder_repo_diff",
      "builder_repo_log",
    ...
    policy: {
      ...
      notes: [
        ...
        "Legacy tool names remain aliased for one rollout to avoid breaking existing prompts and internal Builder instructions.",
      ],
    },
    audit: {
      version: 1,
  ...
  {
    key: "version_control_remote",
    title: "Version Control Remote",
    domain: "version_control",
    tier: "extended",
    status: "available",
    summary: "Remote git management and sync workflows with explicit remote allowlists, audit visibility, and approval-gated push behavior.",
    tools: [
    ...
    policy: {
      ...
      requiresExplicitApproval: true,
      notes: [
        "Remote operations must validate normalized remote URLs against the Builder remote allowlist.",
        "Push and other approval-gated remote mutations should carry explicit operator confirmation and reason metadata.",
      ],
    },
    audit: {
      version: 1,
      eventName: "builder.vcs.remote",
      requiredContext: ["projectId", "taskId", "runId", "timestamp", "actor"],
      scope: "project",
      targets: [
        { kind: "repository", identifier: "builder project repository" },
        { kind: "host", identifier: "normalized remote url" },
      ],
      outcomeStatuses: ["succeeded", "failed", "blocked"],
      redactByDefault: false,
    },
  ...
  {
    key: "container_inspection",
    title: "Container Inspection",
    domain: "runtime",
    tier: "experimental",
    status: "partial",
    summary: "Compose-backed container inspection for discovered Builder services with read-only logs and bounded file checks.",
    tools: [
    ...
    policy: {
      ...
      requiresExplicitApproval: false,
      notes: [
        "Container inspection is limited to project-local compose services discovered by Builder runtime reconciliation.",
        "Managed-container inventory may inspect Builder-labeled containers and legacy Builder test fixtures on the host, but it remains bounded to Builder ownership markers.",
        "In-container reads must stay inside explicitly allowlisted path prefixes and remain read-only.",
      ],
    },
    audit: {
      version: 1,
      eventName: "builder.container.inspect",
      requiredContext: ["projectId", "taskId", "runId", "timestamp", "actor"],
      scope: "project",
      targets: [
        { kind: "service", identifier: "compose service id" },
        { kind: "container", identifier: "compose container id", metadata: ["serviceId"] },
      ],
      outcomeStatuses: ["succeeded", "failed", "blocked", "timed_out"],
      redactByDefault: true,
    },
  ...
  {
    key: "container_execution",
    title: "Container Execution",
    domain: "runtime",
    tier: "experimental",
    status: "partial",
    summary: "Allowlisted in-container test and command execution for discovered compose services with explicit approval.",
    tools: [
    ...
    policy: {
      ...
      requiresExplicitApproval: true,
      notes: [
        "Container execution is bounded to compose-backed services resolved through Builder runtime discovery.",
        "Managed-container removal is host-scoped but only applies to Builder-labeled containers and narrowly identified legacy Builder test fixtures.",
        "Docker-ready stage validation wraps start, inspect, in-container verification, and cleanup into a single Builder-native workflow.",
        "Preset-based test execution is preferred; raw in-container exec remains the allowlisted escape hatch.",
      ],
    },
    audit: {
      version: 1,
      eventName: "builder.container.exec",
      requiredContext: ["projectId", "taskId", "runId", "timestamp", "actor"],
      scope: "project",
      targets: [
        { kind: "service", identifier: "compose service id" },

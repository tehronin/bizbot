      ...
      <section className="w-full max-w-3xl border p-8 space-y-6" style={{ borderColor: "var(--border)", background: "var(--bg-surface)" }}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-3">
            ...
            <input value={googleBusinessClientId} onChange={(event) => setGoogleBusinessClientId(event.target.value)} placeholder="GOOGLE_BUSINESS_CLIENT_ID" className="w-full bg-transparent border px-3 py-2 text-sm" style={{ borderColor: "var(--border)" }} />
            <input value={googleBusinessClientSecret} onChange={(event) => setGoogleBusinessClientSecret(event.target.value)} placeholder="GOOGLE_BUSINESS_CLIENT_SECRET" className="w-full bg-transparent border px-3 py-2 text-sm" style={{ borderColor: "var(--border)" }} />
            <input value={googleBusinessRefreshToken} onChange={(event) => setGoogleBusinessRefreshToken(event.target.value)} placeholder="GOOGLE_BUSINESS_REFRESH_TOKEN" className="w-full bg-transparent border px-3 py-2 text-sm" style={{ borderColor: "var(--border)" }} />
          </div>
          <div className="space-y-3">
            ...
            <input value={googleBusinessAccountName} onChange={(event) => setGoogleBusinessAccountName(event.target.value)} placeholder="GOOGLE_BUSINESS_ACCOUNT_NAME" className="w-full bg-transparent border px-3 py-2 text-sm" style={{ borderColor: "var(--border)" }} />
            <input value={googleBusinessLocationName} onChange={(event) => setGoogleBusinessLocationName(event.target.value)} placeholder="GOOGLE_BUSINESS_LOCATION_NAME" className="w-full bg-transparent border px-3 py-2 text-sm" style={{ borderColor: "var(--border)" }} />
            <input value={googleBusinessInfoLocationName} onChange={(event) => setGoogleBusinessInfoLocationName(event.target.value)} placeholder="GOOGLE_BUSINESS_INFO_LOCATION_NAME" className="w-full bg-transparent border px-3 py-2 text-sm" style={{ borderColor: "var(--border)" }} />

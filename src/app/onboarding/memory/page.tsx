...
export default function OnboardingMemoryPage() {
  ...
  return (
    <main className="min-h-screen px-6 py-8 bg-base text-primary">
      <section className="mx-auto grid w-full max-w-6xl gap-6 lg:grid-cols-[0.7fr_1.3fr]">
        <section className="border p-8 space-y-6 self-start border-border bg-surface">
          <div>
            <div className="text-xs uppercase tracking-[0.24em] mb-4 text-muted">step 4: seed explicit memory</div>
          ...
          <div className="space-y-3 text-xs leading-6 text-dim">
          ...
          <div className="flex gap-3">
            <button onClick={() => void markStep()} className="px-4 py-2 border text-sm uppercase tracking-[0.18em] border-accent text-accent">mark step complete</button>
            <Link href="/onboarding/complete" className="px-4 py-2 border text-sm uppercase tracking-[0.18em] border-border text-primary">next</Link>
          </div>
        </section>

...
export default function OnboardingCompletePage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6 bg-base text-primary">
      <section className="w-full max-w-2xl border p-8 border-border bg-surface">
        <div className="text-xs uppercase tracking-[0.24em] mb-4 text-muted">complete</div>
        <h1 className="text-3xl mb-4">Setup checkpoint reached</h1>
        <p className="text-sm mb-6 text-dim">
        ...
        <div className="space-y-2 text-xs leading-6 mb-6 text-dim">
        ...
        <div className="flex gap-3">
          <button onClick={() => void finish()} className="px-4 py-2 border text-sm uppercase tracking-[0.18em] border-accent text-accent">enter dashboard</button>
          <Link href="/settings" className="px-4 py-2 border text-sm uppercase tracking-[0.18em] border-border text-primary">settings</Link>
          <Link href="/settings#explicit-user-memory" className="px-4 py-2 border text-sm uppercase tracking-[0.18em] border-border text-primary">seed memory</Link>
        </div>
      </section>
    </main>
  );
